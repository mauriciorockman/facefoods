var app = angular.module('webviewApp', ['ui.router', 'ngAnimate', 'templates', 'ui.bootstrap', 'toaster', 'ngResource']);

app.controller('indexCtrl', ['$scope', '$rootScope', '$http', '$timeout', 'toaster', '$state', 'menuFactory', function ($scope, $rootScope, $http, $timeout, toaster, $state, menuFactory) {
    if(menuFactory.setMenu){
        $timeout(function(){$scope.loaded = true;}, 1000);
        $state.go('Menu');
    }else{        
        toaster.pop('error', "Falha", "Falha ao coletar dados, verifique a conexão ou tente novamente mais tarde");
    }
